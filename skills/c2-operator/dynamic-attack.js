...
async function getMitreTechniques(tactic) {
  const res = await request(C2_BASE_URL + "/api/mitre/search/" + tactic);
...
async function cmdAutoAttack(tactic) {
  ...
  for (let i = 0; i < selected.length; i++) {
    ...
    const detailRes = await request(C2_BASE_URL + "/api/mitre/" + tCode);
  ...
  const outputRes = await request(C2_BASE_URL + "/api/output/" + victim.host);
  ...
  const aptRes = await request(C2_BASE_URL + "/api/apt/correlate?top_n=3");
